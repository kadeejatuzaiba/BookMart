...
const cancelProduct = async (req, res) => {
  try {
    ...
    if (!reason)
      return res.status(400).json({ ok: false, msg: 'Reason required' });
...
const returnProduct = async (req, res) => {
  try {
    ...
    if (!reason || !description)
      return res
        .status(400)
        .json({ ok: false, msg: 'Reason & description required' });
    ...
    if (
      order.orderedItems.every((i) =>
        ['Return Request', 'returned'].includes(i.status)
      )
    )
...
const retryCOD = async (req, res) => {
  try {
    ...
    if (order.finalAmount > 1000) {
      return res
        .status(400)
        .json({ message: 'COD is only available for orders up to ₹1000.' });
    }
