...
const placeOrder = async (req, res) => {
  try {
    ...
    if (!userId || !selectedAddress || !paymentMethod) {
      return res
        .status(400)
        .json({ status: false, message: 'Missing order information.' });
    }
    ...
    const regularTotal = cart.items.reduce(
      (sum, i) => sum + i.quantity * i.productId.regularPrice,
      0
    );
...
const verifyPayment = async (req, res) => {
  try {
    ...
    if (generatedSignature !== razorpay_signature) {
      return res
        .status(400)
        .json({ status: false, message: 'Signature mismatch' });
    }
    ...
    const cart = await Cart.findOne({ userId: order.user }).populate(
      'items.productId'
    );
...
const applyCoupon = async (req, res) => {
  ...
  try {
    ...
    if (!subtotal)
      return res.json({ ok: false, msg: 'Could not calculate total' });

    /* 6️⃣  Min order check */
    if (subtotal < coupon.minimumPrice) {
      return res.json({
        ok: false,
        msg: `Minimum order value ₹${coupon.minimumPrice}`,
      });
    }
