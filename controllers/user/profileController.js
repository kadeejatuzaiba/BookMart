  ...
  try {
    ...
    if (findUser) {
      ...
      if (emailSent) {
        ...
        ((req.session.email = email), res.render('forgotPass-otp'));
        console.log('OTP:', otp);
      } else {
        res.json({
          success: false,
  ...
  try {
    ...
    if (!email) {
      return res.status(400).json({
        success: false,
        message:
          'Email not found in session. Please restart password reset process.',
      });
    }
...
const postAddAddress = async (req, res) => {
  try {
    const userId = req.session.user;
    const { name, locality, city, state, pincode, mobile, alternatePhone } =
      req.body;
