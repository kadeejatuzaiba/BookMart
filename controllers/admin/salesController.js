  ...
  try {
    ...
    const [
          ...
          overall: [
            {
              $group: {
                ...
                netRevenue: {
                  $sum: { $subtract: ['$finalAmount', '$totalDiscount'] },
                },
                totalOrders: { $sum: 1 },
                totalProducts: { $sum: { $size: '$orderedItems' } },
              },
      ...
      /* ---- flatten ---- */
      {
        $project: {
          totalSales: {
            $ifNull: [{ $arrayElemAt: ['$overall.totalSales', 0] }, 0],
          },
          totalDiscounts: {
            $ifNull: [{ $arrayElemAt: ['$overall.totalDiscounts', 0] }, 0],
          },
          netRevenue: {
            $ifNull: [{ $arrayElemAt: ['$overall.netRevenue', 0] }, 0],
          },
          totalOrders: {
            $ifNull: [{ $arrayElemAt: ['$overall.totalOrders', 0] }, 0],
          },
          totalProducts: {
            $ifNull: [{ $arrayElemAt: ['$overall.totalProducts', 0] }, 0],
          },
          monthly: 1,
        },
      },
    ...
    const [stats] = await Order.aggregate([
      ...
      /* --------------- flatten --------------- */
      {
        $project: {
          ...
          totalPrice: {
            $ifNull: [{ $arrayElemAt: ['$overall.totalPrice', 0] }, 0],
          },
          totalOrders: {
            $ifNull: [{ $arrayElemAt: ['$overall.totalOrders', 0] }, 0],
          },
